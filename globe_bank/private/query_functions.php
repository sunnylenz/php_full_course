...
function delete_page($id){
    ...
    $old_position = $old_page['position'];
    shift_page_positions($old_position, 0, $page['subject_id'], $id);
...
function count_pages_by_subject_id($subject_id, $options = []) {
  ...
  $sql = "SELECT COUNT(id) FROM pages ";//$sql = "SELECT COUNT(*) FROM pages ";  $sql = "SELECT COUNT(id) AS count FROM pages ";
  $sql .= "WHERE subject_id='" . db_escape($db, $subject_id) . "' ";
  if($visible){
      $sql .= "and visible = true ";
  }
  ...
  $result = mysqli_query($db, $sql);
  confirm_result_set($result);
  $row = mysqli_fetch_row($result);  //$row = mysqli_fetch_assoc($result);
  mysqli_free_result($result);
  $count = $row[0];                 //$count = $row['count']
  return $count;
}
